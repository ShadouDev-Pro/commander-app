...
const createCard = (id, name, mana, colors, type, keywords, text, power, toughness) => {
  const hasKeywordsArray = Array.isArray(keywords);
  return {
    id,
    name,
    mana,
    colors,
    type,
    keywords: hasKeywordsArray ? keywords : [],
    text: hasKeywordsArray ? text || '' : keywords || '',
    image: null,
    power: hasKeywordsArray ? power : text,
    toughness: hasKeywordsArray ? toughness : power,
    rarity: 'common'
  };
};
...
export const sampleDecks = [
  {
    ...
    cards: [
      ...
      createCard('card-1-15', 'Serra Angel', '{3}{W}{W}', ['W'], 'Creature — Angel', ['Flying', 'Vigilance'], 'Flying, vigilance', '4', '4'),
    ],
    colors: ['U'],
    createdAt: new Date(),
    updatedAt: new Date()
  },
  {
    ...
    cards: [
      ...
      createCard('card-2-12', 'Bramblewood Paragon', '{1}{G}', ['G'], 'Creature — Elf Warrior', ['Trample'], 'Trample\nEach other Warrior creature you control enters the battlefield with an additional +1/+1 counter on it.', '2', '2'),
    ],
    colors: ['B'],
    createdAt: new Date(),
    updatedAt: new Date()
  },
  {
    ...
    cards: [
      ...
      createCard('card-3-2', 'Thalia, Guardian of Thraben', '{W}{W}', ['W'], 'Creature — Human Soldier', ['First Strike'], 'First strike\nNoncreature spells your opponents cast cost {1} more to cast.', '2', '1'),
